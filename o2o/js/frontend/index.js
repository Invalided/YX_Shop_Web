...
$(function() {
	...
	var url = server+'/o2o/frontend/listmainpageinfo';
	//访问后台，获取头条列表以及一级类别列表
		...
		if (data.success) {
			...
			headLineList.map(function(item, index) {
				swiperHtml += '' + '<div class="swiper-slide img-wrap">'
						...
						+ '" external><img class="banner-img" src="' + server + item.lineImg
			...
			shopCategoryList.map(function(item, index) {
				categoryHtml += ''
						...
						+ '<img class="shop-img" src="' + server + item.shopCategoryImg
						+ '">' + '</div>' + '</div>';
			});
	...
	$('.row').on('click', '.shop-classify', function(e) {
		...
		var newUrl = server+'/o2o/frontend/shoplist?parentId=' + shopCategoryId;
